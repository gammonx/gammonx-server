# App

## General
- sound/athmosphere
    - maybe variant specific

## Screens

### StartUp
- loading screen
- app update on startup

### Login Screen
- Email/Password
- Login Button
- Forgot Password
- Login With Google/Fb/Apple/Custom
- Create Account Button

### Create Account Screen
- username
- email 
- password
- origin country
- birthdate
- language
- TOS/DSVGO

### Initial Setting Screen
- default match variant

### Play Screen
- bot difficulty (easy, normal, hard)
- header bar
    - name
    - leader board button
    - settings button
- match found experience
    - sound/alert
    - popup
    - accept/decline
- arena modus (ranked)
- game history
- multiple queues streamlining
- ad placement

### Shop Screen
- subscription advertisment
- checkout redirect to native app store flow

### Collection Screen
- tabs for board, dice and checkers
- start with 3 default designs (board, dice, checker) based on match variants
- default designs not persisted in db yet
- activate/deactivate active design
- preview design

### Stat Screen
- show stats for Tavli/Normal/5pts
- all time stats for my profile
- stat list in discord

### Clash (Tournament) Screen
- placeholder/popup/teasing

### Setting Screen/Flyout
- in game settings
    - dice behavior (fast dice)
    - skip animations
    - skip bot turn animation
    - show pip count
    - autobearoff (effort? otherwise innovation board)
    - automove (only 1 move possible)
- sound settings
- change default match variant
- logout
- cancel subscription

### Leaderboard Screen
- match variant filter
- show first 20
- bottom row shows player

### Board Screen
- manual dice roll 

### Match Result Screen
- ranked elo gain/loss
- async lambda queue call (effort? otherwise innovation board)

# Landingpage
- app links
- one pager
- feature list

# Organizational
- Marketing Plan
- Pricing Plan
- Release Strategy

# Innovation Board
- learn component
- analyze component
- friendlist / play a friend
- export matches in given format for external analysis