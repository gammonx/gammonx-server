    ...
    internal class RatingPeriodItemFactory : IItemFactory<RatingPeriodItem>
    {
        ...
        public RatingPeriodItem CreateItem(Dictionary<string, AttributeValue> item)
        {
            var ratingPeriodItem = new RatingPeriodItem
            {
                ...
                PlayerRating = double.Parse(item["Rating"].N),
                PlayerRatingDeviation = double.Parse(item["RatingDeviation"].N),
                PlayerSigma = double.Parse(item["Sigma"].N),
        ...
        public Dictionary<string, AttributeValue> CreateItem(RatingPeriodItem item)
        {
            ...
            var itemDict = new Dictionary<string, AttributeValue>
            {
                ...
                { "Rating", new AttributeValue() { N = item.PlayerRating.ToString() } },
                { "RatingDeviation", new AttributeValue() { N = item.PlayerRatingDeviation.ToString() } },
                { "Sigma", new AttributeValue() { N = item.PlayerSigma.ToString() } },
