...
namespace GammonX.Engine.Models
{
	...
	/// <seealso href="https://www.bkgm.com/rules.html"/>
	/// </summary>
	internal sealed class BackgammonBoardModelImpl : BoardBaseImpl, IHomeBarModel, IDoublingCubeModel, IHitModel
	{
		...
		public void AcceptDoublingCubeOffer(bool isWhite)
		{
			...
			if (owner.HasValue && owner.Value && DoublingCubeValue > 1)
			{
				throw new InvalidOperationException(
					"Doubling offer can only be accepted by a non owner of the doubling cube");
			}
			else
			{
		...
		{
			return other is BackgammonBoardModelImpl otherBoard
			       && HomeBarCountWhite == otherBoard.HomeBarCountWhite
			       && HomeBarCountBlack == otherBoard.HomeBarCountBlack
			       && DoublingCubeValue == otherBoard.DoublingCubeValue
			       && DoublingCubeOwner == otherBoard.DoublingCubeOwner;
		}
		...
		protected override void AddVariantStateHash(ref HashCode hash)
		{
			...
			hash.Add(DoublingCubeValue);
			hash.Add(DoublingCubeOwner);
		}
	}
}
