        ...
        public async Task StartMatchAndGameIntegrationTest(MatchModus modus)
        {
            ...
            player1Connection.On<object>(ServerEventTypes.MatchWaitingForStartEvent, response =>
            {
                ...
                if (contract?.Payload is EventMatchStatePayload payload)
                {
                    ...
                    Assert.Equal(modus, payload.Modus);
            ...
            player1Connection.On<object>(ServerEventTypes.MatchWaitingEvent, response =>
            {
                ...
                if (contract?.Payload is EventMatchStatePayload payload)
                {
                    ...
                    Assert.True(payload.Player1.StartDiceRoll > 0);
                    Assert.Null(payload.Player2.StartDiceRoll);
            ...
            player1Connection.On<object>(ServerEventTypes.MatchStartedEvent, response =>
            {
                ...
                if (contract?.Payload is EventMatchStatePayload payload)
                {
                    ...
                    // player 1 won
                    Assert.True(payload.Player1.StartDiceRoll > payload.Player2.StartDiceRoll);
                }
                else
                {
                    Assert.Fail("Expected EventResponseContract<EventMatchStatePayload> but got: " + response?.GetType().Name);
            ...
            player1Connection.On<object>(ServerEventTypes.GameStartedEvent, response =>
            {
                ...
                if (contract?.Payload is EventGameStatePayload payload)
                {
                    ...
                    // player 1 who won the initial dice roll can directly start moving
                    Assert.Equal(GamePhase.Rolling, payload.Phase);
                    Assert.Equal(1, payload.TurnNumber);
                    // we expect both starting dice rolls 
                    Assert.Equal(2, payload.DiceRolls.Length);
                    Assert.Equal(6, payload.DiceRolls[0].Roll);
                    Assert.False(payload.DiceRolls[0].Used);
                    Assert.Equal(1, payload.DiceRolls[1].Roll);
                    ...
                    // player 1 who starts the game implicitly has his first roll based on the opening rolls
            ...
            {
                // player 1 starts the game with moving
                Assert.NotNull(response);
                var contract = JsonConvert.DeserializeObject<EventResponseContract<EventGameStatePayload>>(response.ToString() ?? "");
                if (contract?.Payload is EventGameStatePayload payload)
                {
            ...
            player1Connection.On<object>(ServerEventTypes.GameStateEvent, response =>
            {
                ...
                if (contract?.Payload is EventGameStatePayload payload)
                {
                    ...
                    {
                        // player 1 rolls the dice implicitly with the match start command
                    }
            ...
            // player 1 must not roll the dice because he starts with moving
